...
Future loadTranslation(Locale locale, bool forceReload) async {
  ...
  final storedLastModifiedDateString =
      await storage.read(key: _translationLastModifiedDateKey(locale));
  final Map<String, dynamic> metaDataInfos =
      jsonDecode(await rootBundle.loadString("translations/metadata.json"));
  final fileLastModifiedDate =
      DateTime.parse(metaDataInfos[locale.languageCode]);

  if ((storedLastModifiedDateString == null || //
      fileLastModifiedDate
          .isAfter(DateTime.parse(storedLastModifiedDateString)) ||
      forceReload)) {
